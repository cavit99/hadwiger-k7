...
## Direct proved input and first missing step
...
The initial diagnostic is `K_{3,3}` with arbitrary coloured schemes.
The required new mechanism must allow branch-set membership to change
between the two bipartition classes, or avoid this packing construction.
No vertex-count or path-length bound is an unbounded reduction.
...
- `K_{3,3}` is already weakly contractible [1, Theorem 5.3]. The diagnostic
  must preserve the prescribed roots; an unrooted model does not settle it.
