...
int main(){const int trials=12000;{
 ...
 for(int t=0;t<trials;t++){G g{15,std::vector<U>(15)};lowK2(g);add(g,0,1); // choose dP(0)=1; C contacts at 0 exactly2
  ...
  std::shuffle(var.begin(),var.end(),rng);int need=53-10; // total m53, fixed low9+boundary1=10 =>43 variable
  for(int i=0;i<need;i++)add(g,var[i].first,var[i].second);
  std::vector<int>C={8,9,10,11,12,13,14}; if(!connected_ind(g,C)||!full(g,C)||pc(g.a[0]&(((U)1<<15)-((U)1<<8)))!=2)continue;built++;if(!c5(g))continue;five++;Search s(g,2000000);int r=s.run();mx=std::max(mx,s.nodes);if(r==1)pos++;else if(r<0)unk++;else{neg++;std::cout<<"NEG K2one\n";break;}}
 ...
 for(int t=0;t<trials;t++){G g{15,std::vector<U>(15)};lowK2(g);for(auto e:std::vector<std::pair<int,int>>{{0,1},{1,2},{1,3},{1,4},{1,5},{2,3},{4,5}})add(g,e.first,e.second);
  ...
  std::shuffle(var.begin(),var.end(),rng);int fixed=9+7;int need=53-fixed;for(int i=0;i<need;i++)add(g,var[i].first,var[i].second);
  if(!connected_ind(g,C)||!connected_ind(g,D)||!full(g,C)||!full(g,D))continue;if(pc(g.a[0]&((U(1)<<8)|(U(1)<<9)|(U(1)<<10)))!=1||pc(g.a[0]&((U(1)<<11)|(U(1)<<12)|(U(1)<<13)|(U(1)<<14)))!=1)continue;built++;if(!c5(g))continue;five++;Search s(g,2000000);int r=s.run();mx=std::max(mx,s.nodes);if(r==1)pos++;else if(r<0)unk++;else{neg++;std::cout<<"NEG K2two\n";break;}}
