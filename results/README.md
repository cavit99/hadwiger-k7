...
| Two-component recolouring forces actual cross-colour edges | [Proof](hc7_common_response_recolouring.md) · [Audit](hc7_common_response_recolouring_audit.md) | Shared α vertices permit repair. In the three-loopless-layer response, every eligible pair instead has a cross-colour edge, and one five-chromatic graph retains all six matching endpoints. The simultaneous minor construction remains open. |
