...
| Colourful sets reduce to three-connected marked minors | [Proof](colourful_five_wheel.md) · [GREEN audit](colourful_five_wheel_audit.md) | For every k>=4, preserve k-chromaticity and a universally colourful set through fixed disjoint marked preimages. At k=5 this gives five colourful-set-rooted wheel bags without connectivity; in six colours any whole class can be reserved. No sixth bag or C19 closure is proved. |
...
| Relative degree and boundary bounds force a five-root wheel | [Proof](five_root_relative_degree_six_wheel.md) · [GREEN audit](five_root_relative_degree_six_wheel_audit.md) | Five roots, at least three nonroots, nonroot degree at least six and every nonempty nonroot subset boundary at least five. No root triangle or pre-existing K4 is required; the hub is unspecified. It supplies no additional exterior contacts. |
| Four-connected graphs excluding `K6-2K2` are four-colourable, except K5 | [Proof](four_connected_five_chromatic_minor.md) · [GREEN audit](four_connected_five_chromatic_minor_audit.md) | No order bound. In a four-connected nonplanar graph, any vertex of degree at least five can be a singleton in Q6; thus Q6-free members are planar or four-regular. The universal-vertex six-chromatic case is closed. General attachment is unproved; novelty is unassessed. |
| Q-free transfers retain the missing pair and expose an actual separator | [Proof](hc7_q_free_rotation_separator.md) · [GREEN audit](hc7_q_free_rotation_separator_audit.md) | Under the two connected-union and adjacent-centre hypotheses, arbitrary transfer components reduce to one gate. A vertex of that gate and three whole row bags separate the two centres from their missing rows; no small vertex-cut bound or C19 closure follows. |
| Three-edge responses retain colouring constraints and admit smaller cores | [Recolouring](hc7_common_response_recolouring.md) · [Audit](hc7_common_response_recolouring_audit.md) · [Reduction](hc7_three_edge_response_reduction.md) · [Audit](hc7_three_edge_response_reduction_audit.md) | Recolouring forces actual cross-colour edges. Allowing marked endpoints to merge gives a three-connected five-critical response with a fixed minor lift; its path case reduces to a triangle. The terminal Q construction remains open. |
