...
Suppose that Theorem~\ref{thm:bilight} is false, and choose a
...
Every use of minimality will preserve $4$-bilightness, the density
bound and the lower bound of three on the order. Models in a minor
lift by replacing each contracted vertex with its fixed connected
preimage.
...
\begin{proof}
Replace an inclusion-maximal reducible fragment by its boundary-rooted
clique or dart. By Lemma~\ref{lem:reduction}, the resulting graph is
a minor, is $4$-bilight and has density at least $\rho(G)$. The
...
removes every interior vertex, and a dart retains one when there were
at least two. This contradicts minimality.
\end{proof}

The next lemma adapts the separation argument of
\cite[Section~5]{DNR2026}. We give the details because the threshold
two and the final $\Kminus$ model are important here.
...
\begin{proof}
We first record the small-separation step. Suppose $(C,D)$ has order
at most four, $R_{C,D}$ is $4$-light and has nonpositive density, and
\[
...
The rooted graph $H=R_{C',D}$ remains $4$-light and of nonpositive
density. This is the root-forgetting observation
...
edges outside a given old fragment, offset by its density cost four.
The roots of $H$ lie in $A$, so $(A\cap D,B)$ is a root separation
of $H$ with right side $R_{A,B}$. Theorem~\ref{thm:helper} gives the
helper model there. It contains one of the rooted graphs allowed in
Lemma~\ref{lem:extraction}; that lemma supplies a reducible fragment
of $H$. This is also reducible in $G$, since at least $|C'|\geq3$
vertices remain outside it. Lemma~\ref{gl:reducible} excludes it.
...
with at most four neighbours. Bilightness makes $R_{A,B}$ $4$-light,
since a positive small-boundary fragment inside $B\setminus A$
...
would again pair with $L$. Also $|L\cup N_G(L)|\geq6$, since $L$
has positive density. Now $B\subseteq D$, contradicting the preceding
small-separation step.
...
Theorem~\ref{thm:helper} applies to the other. Join matching root bags
at their common root. The five joined bags form a clique; the two
helper bags stay in the second open side and retain at least ten of
their eleven possible contacts. This is a $\Kminus$ model. Thus
...
their closed sides can be joined by five disjoint paths. Otherwise
...
$R_{C,D}$ proves its $4$-lightness. The small-separation step, now
applied to $R_T$, gives a contradiction. The five paths therefore exist.
...
in $R_T$, using (2). Join corresponding root bags along these paths.
Their interiors are disjoint and assigned to their own joined bag,
so all bags remain disjoint and connected. Again they give $\Kminus$.
...
\begin{proof}
If $d_G(v)\leq4$, the smaller minor $J=G-v$ has density at least $-2$
...
clique directly. For $k=3$, use Lemma~\ref{lem:triangle}; for $k=4$,
use Lemma~\ref{lem:dart}. In the last case $|A|\geq2$, since the
...
Write $t(xy)=|N_G(x)\cap N_G(y)|$ for an edge $xy$. Contracting it
...
We will use one further observation repeatedly. In a five-connected
graph, contraction of an edge gives a four-connected graph, and every
...
the merged vertex belongs to both boundaries. Its open sets lift to
...
order at most five. Lemma~\ref{gl:orientations}(3) therefore lets
us choose $Y$ with boundary five and $\rho_G(Y)=1$. Its boundary
...
The connected bags $\{a,v\}$ and $\{c,b\}$ are adjacent to each
...
Now $t(av)=0$. The contraction $G/av$ has sufficient density, so
...
This final contraction contradicts minimality. All contractions
used above leave at least eight vertices, so the order condition
...
Lemma~\ref{lem:star} to $H$ rooted at these ends. Its central bag
avoids $S$. Use this bag and $\{v\}$ as helpers. Four of the
linkage paths become root bags; delete the last vertex of the
central path to obtain the fifth root bag. This trimmed path is
nonempty, since its end lies outside $X_0$ and hence outside $S$.
Both helpers meet the other four root bags, the helpers are adjacent,
and the trimmed bag meets the central helper. Thus at least ten
of the eleven helper contacts are present. Every bag lies in
$A\cup S$, and no artificial boundary edge has been used.
...
Joining corresponding root bags gives $\Kminus$, a contradiction.
\end{proof}

The following elementary end argument will exclude the remaining
...
$(A\cap T)\cup(S\cap T)\cup(S\cap B)$. This containing set cannot
have order below five, by connectivity. If it has order five, it
equals the actual boundary and contains both ends of $e$, giving
an $F$-fragment strictly inside $A$. Its opposite side contains
...
In these applications a nonempty opposite corner witnesses that
the candidate separation is proper.
...
$w=|D|\geq4$. This contradicts $u+w\leq5$ in every case.
\end{proof}
...
allows only densities at most one or at least seven. Notice also
that a low five-fragment is automatically proper: an empty opposite
side would give density $18-e(G[N_G(A)])\geq8$.
...
Lemma~\ref{gl:orientations}(3) forces one density to equal one.
...
The degree-six argument adapts the final part of
Norin--Totschnig's proof in~\cite[Section~3]{NT2025}; here
six-connectivity excludes the separator outcome directly. It uses
...
path into one of its ends fills two of the three possible missing
contacts. Together with $\{v\}$, the six resulting bags give
$\Kminus$. In particular all three pairs are nonedges. Otherwise
an existing pair edge and a path for another pair through an exterior
...
Hence $H'$ has the disc drawing. Its four distinct boundary
terminals give $e(H')\leq3v(H')-7$. Every vertex outside $N_H[v]$
...
Our minimum counterexample $G$ is six-connected by
...
Dvo\v r\'ak, Norin and Rahman~\cite[Theorem~1.6]{DNR2026} give
