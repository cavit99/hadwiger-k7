...
We begin with three rooted constructions and a local contraction lemma,
all used again in the unrooted argument.

The rooted-star lemma is related to~\cite[Lemma~4.10]{DNR2026}, which
gives three spokes at one of five roots when $|V(H)|+|X|\le8$.
Under the degree assumptions below, independence of $X$ allows all
$k-1$ spokes and the bound $|V(H)|+|X|\le2k-1$.
...
\begin{proof}
Put $T=V(H)\setminus Z$. A component $D$ of $H[T]$ is adjacent to
all but at most $|D|$ vertices of $Z$, since every vertex of $D$ has
at least $k-|D|$ neighbours in $Z$. Thus at most $|T|$ roots have no
neighbour in some component of $H[T]$. Since $|T|+|X|\le k-1$,
there is a vertex
$z\in Z\setminus X$ adjacent to every component. Thus $T\cup\{z\}$
is connected, including when $T=\varnothing$.

A vertex $r\in Z\setminus\{z\}$ with no neighbour in $T$ must
...
third path is the branch set at $c$. These three branch sets are
...
If the median of the three root nodes is a block node, the roots have
three distinct attachments to that block. The block is two-connected,
so the preceding observation supplies a triangle minor rooted at the
attachments. Appending the three disjoint exterior root paths gives
the result.
...
contained in $c$ and the possible root, contrary to internal
...
then the graph has a rooted dart. This part of the cited proof requires
only these tree and path conditions; no density hypothesis is needed.
...
\begin{proof}
Write $(F,Z)$ for the rooted graph and proceed by induction on $|V(F)|$.
Delete the root--root edges; this preserves all nonroot degrees and
boundaries.
...
Each component of $F-Z$ is adjacent to all four roots, so it supplies a
...
$v_2$ into one root branch set: the resulting root star contains the
required three-vertex path.
...
roots. Append these segments to the branch sets at $v_1,v_2$. They are
disjoint from the other branch sets and the helper, and restore the four
original roots while preserving every required adjacency.
\end{proof}
...
\begin{proof}
Since the graph is five-connected, an edge contraction preserves
four-connectivity. A forbidden four-separation of $Q/xa$ would lift to
a five-cut $T$ of $Q$ containing
...
\section{The rooted theorem}\label{sec:helper}

For five prescribed roots $X$, say that a model has the
\emph{helper property} if it consists of five branch sets at the
prescribed roots and two further nonempty connected branch sets, all
pairwise disjoint, with at least ten of the eleven possible helper--root
and helper--helper adjacencies. The two helpers contain no root;
no root--root adjacency is required.
...
We first establish structural properties of a minimum counterexample,
then use a weighted neighbourhood to find an edge with few common
neighbours. An auxiliary clique allows us to apply Mader's atom theorem
without requiring intersections of positive-density fragments to have
positive density.

\subsection{Minimum-counterexample reductions}

Suppose the theorem is false, and choose a counterexample minimising
$(|V(G)|,|E(G)|)$ lexicographically. Throughout this section, the preimage
of each vertex under a contraction contains at most one original root.
A rooted model in a minor lifts by replacing each vertex with its fixed
connected preimage; in particular, all five prescribed roots remain
distinct.
...
or two helpers adjacent to every branch set at a larger boundary.
Either five disjoint exterior paths attach the original roots to five
boundary branch sets, or a separator of order $k\le4$ isolates the
region and has a linkage to $k$ of its boundary branch sets. A helper
model restricted to those $k$ branch sets contains a rooted $K_k$ for
$k\le3$, and a rooted dart for $k=4$. Indeed, retain a helper adjacent
to all four root branch sets and absorb the other into a root branch
set to obtain two root--root adjacencies. For three roots, absorb the
helpers into two root branch sets; smaller cases follow by restriction.
The same conclusion holds when both helpers are adjacent to every branch
set at a larger boundary. If the isolated interior has at least two
vertices and nonpositive density, it is a reducible fragment. Apart from
its ends, the linkage lies outside the original region and hence avoids
the helpers. This is the only form of the isolator argument needed below.
...
helper adjacency, and cannot occur inside a branch set containing just
one root. Thus minimality makes $X$ independent. An inclusion-maximal
reducible fragment would give, by \cite[Lemma~3.3]{DNR2026}, a smaller
$4$-light rooted
...
The densities of the components of $G-X$ sum to $\rho_4(G,X)$. By
$4$-lightness, each component of positive density is adjacent to every
root. Two such components give two helpers adjacent to all five
roots, with only the adjacency between the helpers possibly missing.
Hence there is exactly one component of positive density, and its
density is at least two. Any other component would make this a proper
five-boundary set, contrary to (2). Thus $G-X$ is connected and adjacent
to every root.
...
\]
Each $Q_i$ is adjacent to $z$, and each component of positive density
is adjacent to at least four roots. If $d=5$, there is a component
of positive density; it and $\{z\}$ are adjacent helpers with at least
ten required adjacencies. If $d=4$, let $x$ be the missing root. A
component adjacent to every root already gives the required model.
Otherwise each component of positive density has exactly four root
neighbours, and has density at most one by (2). These densities sum to
at least two, so at least two components have positive density. Choose
a component $R$ adjacent to $x$, and a component $Q\ne R$ of
positive density. The helpers $\{z\}\cup R$ and $Q$ are adjacent and
have respectively five and at least four root adjacencies. This again
...
Thus deletion stays in the induction class, contradicting minimality.
...
graph $(G[Y\cup S],S)$ is internally $k$-connected, retains all nonroot
...
completion counts exactly the edges that become root--root edges when
...
Delete $p,q$ from this side and reroot at $\{u,v\}\cup D$:
\[
...
five rooted branch sets in $R$, with those at $u,v$ adjacent to one
another and to all three branch sets at $D$.

Append $p$ to the branch set at $u$ and $q$ to the branch set at $v$.
These give adjacent root-free helpers, each adjacent to every branch
set at the boundary $A\cup D$. For $A\cap D$ use the existing branch
sets at $D$; for $A\setminus D$ use singletons. These branch sets are
disjoint because $T\cap A=\varnothing$.

The root-free set $U=T\cup\{p,q\}$ has boundary exactly $A\cup D$.
If five exterior paths link $X$ to this boundary, retain the five branch
sets at their distinct ends and append the paths. This gives the helper
...
five linkage paths as root branch sets, removing $z$ from its own path.
The trimmed branch set is nonempty because $z$ is not an original root.
The helper $z$ is adjacent to all five root branch sets, $v$ is adjacent
to the other four, and $vz$ joins the helpers. Every clique edge used
has nonroot end $z$, so is an edge of $G$. This gives the ten required
adjacencies.
\end{proof}
...
The trimmed root branch set is nonempty, and all branch sets are
disjoint. The helper $C$ has five root adjacencies, $v$ has four, and
they are adjacent. Thus the model has the helper property, a contradiction.
\end{proof}
...
Adding this clique changes neither boundaries of original root-free
sets nor common-neighbour counts of original edges with a nonroot end.

Let $\mathcal S=\{V(e):e\in E_0\}$. By Lemma~\ref{rh:blocker},
$\mathcal S$-fragments exist in $Q$, with an example of order at most
$m=|V(G)\setminus X|$. Choose an $\mathcal S$-atom $A$, so
$|A|\le m$. It avoids $X\cup W$: otherwise its five-boundary would
force it to contain the whole surviving clique,
...
We used the auxiliary clique only to select an atom; no model was
constructed in the enlarged graph. The contradiction proves the theorem
for the original rooted graph.
\end{proof}
